...
import net.aixin.app.api.OperationResponseHandler;
...
public class ServerTaskService extends IntentService {
	...
	}else if (ACTION_TECHNICAL_SEND_POST.equals(action)) {//发帖
		TechnicalBean bean = intent.getParcelableExtra(BUNDLE_TECHNICAL_SEND);
		if (bean != null) {
			putTechnical(bean);
		}
	}
	...
	//*******
	//发帖
	public static final String ACTION_TECHNICAL_SEND_POST = "com.aixintrip.technical.send.post";
	public static final String BUNDLE_TECHNICAL_SEND = "BUNDLE_TECHNICAL_SEND";
	public static final String KEY_TECHNICAL = "technical_";

	public void putTechnical(TechnicalBean bean) {
		bean.setId((int) System.currentTimeMillis());
		int id = bean.getId();
		addPenddingTask(KEY_TECHNICAL + id);

		notifySimpleNotifycation(id, "正在发帖",
				"发帖",
				"正在发帖", true, false);

		OSChinaApi.send_post(bean, new PublicTechniaclResponseHandler(getMainLooper(), bean, KEY_TECHNICAL));
	}
	//发帖handler
	class PublicTechniaclResponseHandler extends OperationResponseHandler {
		String key = null;

		public PublicTechniaclResponseHandler(Looper looper, Object... args) {
			super(looper, args);
			key = (String) args[1];
		}
		...
		public void onSuccess(int code, ByteArrayInputStream is, Object[] args) throws Exception {
			super.onSuccess(code, is, args);
			TechnicalBean bean = (TechnicalBean) args[0];
			int id = bean.getId();

			//转出成javaBean结果实体
			if (true) {
				notifySimpleNotifycation(id,"发帖成功","发帖","发帖成功", false, true);
				removePenddingTask(key + id);
			} else {
				onFailure(100, "服务器返回的错误提示", args);
			}
		}
		...
		public void onFailure(int code, String errorMessage, Object[] args) {
			...
			notifySimpleNotifycation(id,
					"发帖失败",
					"发帖",
					code == 100 ? errorMessage : "发帖失败了", false,
					true);
			removePenddingTask(key + id);
		}
